...
if(isset($_POST["comment"])&&isset($_POST["blogid"]))
	{
		...
		{	
			if(!empty($_SESSION['userID']))
			  {
			  	echo "insert into `article_comment`(`comment`,`articleid`,`userid`) values(\"".$comment."\",\"".$blogid."\",\"".$_SESSION['userID']."\")";
	           $query=mysql_query("insert into `article_comment`(`comment`,`articleid`,`userid`) values(\"".$comment."\",\"".$blogid."\",\"".$_SESSION['userID']."\")");
					...
					if($rowno>0)
					{
						...
						while($db_field_id = mysql_fetch_assoc($query_run))
						{
							...
							echo '<div class="news-comment" id="news-comment'.$i.'" name="'.$i.'">';
								...
								echo '<img class="blog-candidate-img" src="../../uploads/'.$run2['pic'].'"/>
						...
						echo '<input type="button" class="news-button-submit"  id="'.$db_field['articleid'].'" name="'.$i.'" value="Post"/>';					
